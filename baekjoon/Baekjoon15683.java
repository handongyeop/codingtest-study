...
public class Baekjoon15683 {
    private static int N, M, res, ccCnt;
    private static CCTV[] cctvs;
    private static int[] dr = {-1, 0, 1, 0}, dc = {0, 1, 0, -1}; // 방향
    private static int[][][] ccDir = { // 각 cctv가 볼 수 있는 영역(회전)
            {{0}},
            {{1}, {2}, {3}, {0}}, // 1번 cctv
            {{1, 3}, {0, 2}}, // 2번 cctv
    ...
    private static void process(int idx, int remain, int[][] map) {
        ...
        for (int i = 0; i < ccDir[cc.type].length; i++) {
            // 해당 CCTV가 감시할 수 있는 방향으로 감시
            int check = 0;
            // 현재 상태에서 감시할 수 있는 방향
            for (int j = 0; j < ccDir[cc.type][i].length; j++) {
                int d = ccDir[cc.type][i][j];
